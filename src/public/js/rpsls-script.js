...
const dom = {
    ...
    playAreaBackground: document.querySelector('.playArea__main-background-rpsls'), 
...
dom.lizard.addEventListener('click', function(){playGame('lizard')});
dom.spock.addEventListener('click', function(){playGame('spock')});
...
async function playGame(pick) {
    ...
    const prevClasses = {
        paper: 'topRight-small-rpsls', 
        scissors: 'topCenter-small-rpsls',
        rock:  'bottomRight-small-rpsls', 
        lizard: 'bottomLeft-small-rpsls', 
        spock: 'topLeft-small-rpsls'
    } 
    ...
    if (winners[pick].includes(house)) {
        curScore ++
...
function choiceDisplay(choice) {
    ...
    let choices = ['rock', 'paper', 'scissors', 'lizard', 'spock'].filter(el=> el !== choice);  
